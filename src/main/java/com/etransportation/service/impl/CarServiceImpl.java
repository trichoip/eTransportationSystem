...
public class CarServiceImpl implements CarService {
  ...
  public void save(CarRegisterRequest carRegisterRequest) {
    Account account = accountRepository
      .findById(carRegisterRequest.getAccount().getId())
    ...
    Ward ward = wardRepository.findById(carRegisterRequest.getWard().getId()).orElseThrow(() -> new IllegalArgumentException("Chưa nhập địa chỉ"));
  ...
  public CarDetailInfoResponse findCarDetailInfo(Long carId) {
    ...
    carDetailInfoResponse.setAddressInfo(car.getAddress().getDistrict().getName() + ", " + car.getAddress().getCity().getName());
    carDetailInfoResponse.getBooks().removeIf(book -> book.getEndDate().before(cal.getTime()) || book.getStatus().equals(BookStatus.CANCEL));
    ...
    carDetailInfoResponse.setTotalRating(
      Double.valueOf(new DecimalFormat("#0.0").format(review.stream().mapToInt(r -> r.getStarReview()).average().orElse(0.0)))
    );
  ...
  public List<CarShortInfoResponse> findAllCarByUserId(Long id) {
    ...
    List<CarShortInfoResponse> listCarInfoResponse = cars
      ...
      .map(c -> {
        ...
        carInfoResponse.setAddressInfo(c.getAddress().getDistrict().getName() + ", " + c.getAddress().getCity().getName());
  ...
  public Object findAllCarByAdmin(PagingRequest pagingRequest) {
    Pageable pageable = PageRequest.of(pagingRequest.getPage() - 1, pagingRequest.getSize(), Sort.by("registerDate").descending());
    ...
    List<CarShortInfoResponse> listCarInfoResponse = cars
      ...
      .map(c -> {
        ...
        carShortInfoResponse.setAddressInfo(c.getAddress().getDistrict().getName() + ", " + c.getAddress().getCity().getName());
        ...
        if (c.getCarImages().size() == 0) {
          ...
          // .get(new Random().nextInt(c.getCarImages().size()))
          // .getImage());
          carShortInfoResponse.setCarImage(c.getCarImages().get(0).getImage());
        }

        carShortInfoResponse.setTotalBook(c.getBooks().size());
        List<Review> review = reviewRepository.findAllByBook_Car_Id(c.getId());

        carShortInfoResponse.setTotalRating(
          Double.valueOf(new DecimalFormat("#0.0").format(review.stream().mapToInt(r -> r.getStarReview()).average().orElse(0.0)))
        );
  ...
  public void carBrowsing(CarBrowsingRequest carBrowsingRequest) {
    Car car = carRepository.findById(carBrowsingRequest.getId()).orElseThrow(() -> new IllegalArgumentException("Car not found"));
  ...
  public Object filterCar(FilterCarSearchRequest filter, PagingRequest pagingRequest) {
    ...
    pagingResponse.setCarModels(carRepository.findAllModelAndCountByFilterSearch(listIdCarModels, filter.getBrand_Id()));
    ...
    pagingResponse.setCarBrands(carRepository.findAllBrandAndCountByFilterSearch(listIdCarBrands));
  ...
  public void updateCar(CarUpdateInfoRequest carInfo) {
    Car car = carRepository.findById(carInfo.getId()).orElseThrow(() -> new IllegalArgumentException("Car not found"));
    Ward ward = wardRepository.findById(carInfo.getWard().getId()).orElseThrow(() -> new IllegalArgumentException("Vui lòng nhập địa chỉ"));
    modelMapper.map(carInfo, car);
    car.getCarImages().clear();
    car.getCarImages().addAll(modelMapper.map(carInfo.getCarImagesUpdate(), new TypeToken<List<CarImage>>() {}.getType()));

    car.getFeatures().clear();
    car.getFeatures().addAll(modelMapper.map(carInfo.getFeaturesUpdate(), new TypeToken<Set<Feature>>() {}.getType()));
  ...
  public void deleteCar(Long id) {
    ...
    switch (car.getStatus()) {
      ...
      case DENIED:
        ...
        break;
      case ACTIVE:
        Boolean checkBooks = carRepository.existsByIdAndBooks_EndDateGreaterThanEqualAndBooks_Status(id, cal.getTime(), BookStatus.SUCCESS);
        if (checkBooks) {
  ...
  public Object getAllReviewByCarId(Long id, PagingRequest pagingRequest) {
    Pageable pageable = PageRequest.of(pagingRequest.getPage() - 1, pagingRequest.getSize(), Sort.by("reviewDate").descending());
    Page<Review> reviews = reviewRepository.findAllByBook_Car_Id(id, pageable);

    List<ReviewByCarResponse> listReviewByCarResponse = reviews
      .getContent()
      .stream()
      .map(r -> {
    ...
    PagingResponse<ReviewByCarResponse> pagingResponse = PagingResponse
      ...
      .totalStarAverage(
        Double.valueOf(new DecimalFormat("#0.0").format(listReviewByCarResponse.stream().mapToInt(r -> r.getStarReview()).average().orElse(0.0)))
      )
      .build();
